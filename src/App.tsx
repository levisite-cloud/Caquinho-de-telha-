...
export default function App() {
  ...
  // --- CÁLCULO CENTRALIZADO DO CAIXA ---
  const totalSuprimentosCaixa = caixaMovimentacoes.filter(m => m.tipo === 'Suprimento').reduce((acc, curr) => acc + Number(curr.valor), 0);
  const totalSangriasCaixa = caixaMovimentacoes.filter(m => m.tipo === 'Sangria').reduce((acc, curr) => acc + Number(curr.valor), 0);
  const saldoFisicoGaveta = caixaAtivo ? Number(caixaAtivo.fundo_inicial) + totalSuprimentosCaixa - totalSangriasCaixa : 0;
        ...
        <div className="fixed inset-0 z-50 bg-zinc-950/80 backdrop-blur-xs flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-[#121214] rounded-xl shadow-2xl border border-zinc-800 w-full max-w-md p-6">
            ...
            <div className="bg-[#1A1A1E] border border-zinc-800/80 rounded-lg p-4 mb-5 flex justify-between items-center">
              <div>
                <span className="text-[11px] text-zinc-500 font-bold uppercase tracking-wider block mb-1">Saldo Atual em Caixa</span>
                <span className="text-xl font-bold text-zinc-100 font-mono">R$ {saldoFisicoGaveta.toFixed(2)}</span>
              </div>
